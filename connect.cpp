...
int find_connected_components_BFS(int* FirstVertex, int V, int* EdgeList, int E, int* Component)
{
    ...
    for (int kk = 0; kk < V; kk++) {
        if (Component[kk] < 0) {
            ...
            /*int x = FirstVertex[kk];
            int y = FirstVertex[kk + 1];
            while (x < y) {
                int n = EdgeList[x];
                if (Component[n] < 0) {
                    Component[n] = NCC_BFS;
            ...
            while (!Q.empty()) {
                int a = FirstVertex[Q.front()];
                int b = FirstVertex[Q.front()+1];
                while (a < b) {
                    int n = EdgeList[a];
                    if (Component[n] < 0) {
                        Component[n] = NCC_BFS;
            ...
            for (int x = FirstVertex[kk]; x < FirstVertex[kk + 1]; x++)
            {
                Component[EdgeList[x]] = NCC_BFS;
                Q.push(EdgeList[x]);
            }

            while (!Q.empty()) {
                for (int x = FirstVertex[Q.front()]; x < FirstVertex[Q.front() + 1]; x++) {
                    int n = EdgeList[x];
                    if (Component[EdgeList[x]] < 0) {
                        Component[EdgeList[x]] = NCC_BFS;
                        Q.push(EdgeList[x]);
                    }
                }
...
void DFS(int v, int* FirstVertex, int V, int* EdgeList, int E, int* Component, int NCC) {
    ...
    for (int e = FirstVertex[v]; e < FirstVertex[v + 1]; e++) {
        ...
        if (Component[n] < 0) {
            Component[n] = NCC;
            DFS(n, FirstVertex, V, EdgeList, E, Component, NCC);
        }
    }
}
